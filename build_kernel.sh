...
if [ -f $KERNELDIR/arch/x86/boot/bzImage ]; then
	mkdir $KERNELDIR/modules 2>/dev/null && make INSTALL_MOD_PATH="$KERNELDIR/modules" modules_install
    mkdir $KERNELDIR/headers 2>/dev/null && make headers_install INSTALL_HDR_PATH="$KERNELDIR/headers"
    mkdir $KERNELDIR/perf 2>/dev/null && make -C tools/perf NO_JEVENTS=1 NO_JVMTI=1 NO_LIBTRACEEVENT=1 install DESTDIR="$KERNELDIR/perf" prefix=/
    sudo ./Microsoft/scripts/gen_artifacts_vhdx.sh "$KERNELDIR/modules" "$KERNELDIR/headers" "$KERNELDIR/perf" $(make -s kernelrelease) $READY_ZIP/AGNi_WSL2_$DEVICE-$AGNI_VERSION-$AGNI_KERNEL_LINUX-modules.vhdx
	...
	if [ -f AGNi_WSL2_$DEVICE-$AGNI_VERSION-$AGNI_KERNEL_LINUX.7z ]; then
		sudo rm AGNi_WSL2_$DEVICE-$AGNI_VERSION-$AGNI_KERNEL_LINUX-bzImage.kernel AGNi_WSL2_$DEVICE-$AGNI_VERSION-$AGNI_KERNEL_LINUX-modules.vhdx
	fi
